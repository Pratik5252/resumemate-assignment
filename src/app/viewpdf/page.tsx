...
const Page = () => {
    ...
    return (
        <div className="bg-white relative w-full h-screen flex flex-col gap-4 items-center justify-start">
            ...
            <div className="w-[90vw] sm:w-[80vw] md:w-[70vw] lg:w-[50vw] flex flex-col gap-4 border px-6 py-10 shadow-lg mt-36">
                {Object.entries(formData).map(([key, value]) => (
                    <div
                        key={key}
                        className="grid grid-cols-[150px_1fr] gap-4 items-baseline"
                    >
                        <h2 className="text-black text-sm sm:text-base font-semibold capitalize">
                            {key.replace(/([A-Z])/g, ' $1')}:
                        </h2>
                        <p className="text-xs sm:text-sm text-stone-600 font-medium w-[80%]">
                            {value ? value : '-'}
                        </p>
                    </div>
                ))}
            </div>
            <div className="w-[90vw] sm:w-[80vw] md:w-[70vw] lg:w-[50vw]">
